...
export const generateExportReport = (logs: LogEntry[]): ExportReport => {
  ...
  const networkLogs = logs.filter(
    (l) => l.type === 'request' || l.type === 'response' || (l.type === 'error' && l.url)
  );
  ...
  const responseTimes = networkLogs
    .filter((l) => l.durationMs !== undefined && l.durationMs !== null)
    .map((l) => l.durationMs!);
  ...
  networkLogs.forEach((l) => {
    ...
    if (l.status !== undefined) {
      ...
      if (l.status >= 200 && l.status < 400) successCount++;
      else if (l.status >= 400) errorCount++;
    }
...
export const formatReportAsText = (report: ExportReport): string => {
  const lines: string[] = [];
  const sep = '='.repeat(60);
  const sub = '-'.repeat(40);
  ...
  report.timeline.slice(0, 50).forEach((t) => {
    const time = new Date(t.timestamp).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit', second: '2-digit' });
