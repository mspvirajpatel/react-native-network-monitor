...
const DEFAULT_CONFIG: Partial<StateMonitorConfig> = {
  throttleMs: 200,
  maxDepth: 3,
  ignoreActions: [
    '@@redux/INIT',
...
function truncateArray(arr: any[], max: number): any[] {
  return arr.slice(0, max).map((item) => truncateValue(item, max));
}

function truncateValue(value: any, maxItems: number, depth = 0, maxDepth = 3): any {
...
function computeDiff(
  ...
  next: any,
  ignoreKeys: string[] = []
): Record<string, { prev: any; next: any }> {
  ...
  for (const key of allKeys) {
    ...
    if (prevObj[key] !== nextObj[key]) {
      diff[key] = { prev: prevObj[key], next: nextObj[key] };
    }
  }
  return diff;
}
...
export function subscribeToState(config: {
  ...
  const flush = () => {
    ...
    if (mode === 'diff') {
      ...
      if (keys.length > 0) {
        const filtered: Record<string, { prev: any; next: any }> = {};
        for (const key of keys) {
          const val = diff[key];
...
export function createReduxMiddleware(options?: {
  ...
  return (store: any) => (next: any) => (action: any) => {
    const shouldIgnore = (config.ignoreActions || []).some((pattern: string) => {
      if (pattern.endsWith('*')) {
    ...
    if (
      ...
      config.includeActions.length > 0 &&
      !config.includeActions.includes(action.type)
    ) {
      return next(action);
    }
    ...
    if (snapshotMode) {
      ...
      if (Object.keys(diff).length > 0) {
        const snapshotSize = JSON.stringify({ prevState, nextState })?.length || 0;
        ...
        Logger.logAction({
          ...
          snapshot: useDiff
            ? truncateValue(nextState, 50, 0, maxDepth)
            : undefined,
        });
      }
    }
...
export function createZustandMonitor(
  storeOrApi: any,
  options?: {
    name?: string;
    mode?: 'diff' | 'snapshot';
