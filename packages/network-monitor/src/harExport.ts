...
const extractQueryString = (url: string): { name: string; value: string }[] => {
...
const headersToArray = (headers: unknown): { name: string; value: string }[] => {
...
const getMimeType = (headers: unknown): string => {
...
const approximateByteSize = (data: unknown): number => {
...
const statusTextFromCode = (status: number): string => {
 ...
 */
export function generateHarExport(logs: LogEntry[]): HarExport {
